...
class Point(object):
    ...
    def move_by(self, dx, dy):
        ...
        self.x = self.x + dx
        self.y = self.y + dy
        self.distance = self.distance + self.get_distance_from(Point(self.newx,self.newy))
        self.moves = self.moves + 1
    def get_number_of_moves_made(self):
        return self.moves
    def get_distance_from(self, point2):
        distance = m.sqrt((((point2.x-self.x)**2)+((point2.y-self.y)**2)))
        return distance
    def get_distance_from_start(self):
        self.distance = self.distance + self.get_distance_from(Point(self.start_x,self.start_y))
    def get_distance_traveled(self):
        return self.distance
    def closer_to(self, point1, point2):
        a = self.get_distance_from(point1)
        b = self.get_distance_from(point2)
        if a<b:
            return point1
        elif b<a:
            return point2
            ...
            return point1
    def halfway_to(self, pointnew):
        pointreturn = Point((pointnew.x+self.x)/2,(pointnew.y+self.y)/2)
        return pointreturn
def run_test_init():
    """
